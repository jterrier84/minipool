...
CNVERSION="1.34.1"                                              ## Must match with the version of the docker image
CNNETWORK="mainnet"                                             ## Use "testnet" if connecting node to the testnet
...
CN_CONFIG_PATH="/home/julienterrier/Cardano/node/files"         ## Path to the folder where the Cardano config files are stored on the host system
CN_DB_PATH="/home/julienterrier/Cardano/node/db"                ## Path to the folder where the Cardano database (blockchain) will be stored on the host system
...
##---------------------------------
docker run --detach \
    --name=cardano-node-mainnet-${CNVERSION} \
    --restart=always \
    -p ${CNPORT}:${CNPORT} \
    ...
    jterrier84/armada-cn:${CNVERSION} \
